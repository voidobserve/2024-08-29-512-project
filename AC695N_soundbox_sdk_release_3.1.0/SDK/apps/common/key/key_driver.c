...
_scan_end:
    scan_para->last_key = cur_key_value; // 保存上一次扫描到的键值
    return;
}
...
// 注册rf遥控器按键的定时扫描(注册rf按键扫描定时器)：
#if RFKEY_ENABLE
    // extern struct key_driver_para rfkey_scan_para;                                            // 按键驱动扫描参数列表
    sys_s_hi_timer_add((void *)&rfkey_scan_para, key_driver_scan, rfkey_scan_para.scan_time); // 注册rf按键扫描定时器
#endif                                                                                        // #if RFKEY_ENABLE
